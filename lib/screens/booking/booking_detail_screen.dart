...
class _BookingDetailScreenState extends State<BookingDetailScreen> with WidgetsBindingObserver {
            ...
            RichText(
              text: TextSpan(
                children: [
                  ...
                  if (res.handymanData.validate().isNotEmpty && (res.providerData!.id == res.handymanData!.first.id.validate()))
                    TextSpan(
  ...
  Widget _action({required BookingDetailResponse bookingResponse}) {
    final isShopBooking = bookingResponse.service?.visitType?.trim().toLowerCase() == VISIT_OPTION_ON_SHOP;
  ...
  Widget _buildShopActions(BookingDetailResponse bookingResponse) {
    ...
    switch (bookingResponse.bookingDetail!.status) {
      ...
      case BookingStatusKeys.accept:
        return bookingResponse.handymanData.validate().isNotEmpty ? _startButton(bookingResponse) : Offstage();
      ...
      case BookingStatusKeys.complete:
        if ((detail.type != SERVICE_TYPE_FREE || detail.paymentMethod == PAYMENT_METHOD_COD) && detail.paymentId == null) {
  ...
  Widget _buildOnSiteActions(BookingDetailResponse bookingResponse) {
    ...
    if ((bookingResponse.service?.isAdvancePayment ?? false) &&
        ...
        (detail.paymentStatus == null || (detail.paymentStatus == SERVICE_PAYMENT_STATUS_ADVANCE_PAID && detail.status == BookingStatusKeys.complete))) {
  ...
  Widget _cancelButton(BookingDetailResponse bookingResponse) => AppButton(
        ...
        onTap: () => _handleCancelClick(
          ...
          isDurationMode: checkTimeDifference(
            inputDateTime: DateTime.parse(bookingResponse.bookingDetail!.date.validate()),
          ),
        ),
      );
  ...
  Widget _payNowOrAdvanceButton(BookingDetailResponse bookingResponse, BookingData detail) => AppButton(
        text: detail.paymentStatus == SERVICE_PAYMENT_STATUS_ADVANCE_PAID && detail.status == BookingStatusKeys.complete ? language.lblPayNow : language.payAdvance,
        textColor: Colors.white,
        color: Colors.green,
        onTap: () => PaymentScreen(bookings: bookingResponse, isForAdvancePayment: true).launch(context),
      );
  ...
  Widget _payNowButton(BookingDetailResponse bookingResponse) => AppButton(
        ...
        textColor: Colors.white,
        color: Colors.green,
        onTap: () => PaymentScreen(bookings: bookingResponse, isForAdvancePayment: false).launch(context),
      );

  Widget _requestInvoiceButton(BookingDetailResponse bookingResponse) => AppButton(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), // Space around content
                      ...
                      if (snap.data!.isBookedAtShop) shopDetailWidget(shop: snap.data!.shop!),
                      ...
                      locationTrackWidget(
                        ...
                        snap.data!,
                      ).visible(BookingStatusKeys.onGoing == snap.data!.bookingDetail!.status && !snap.data!.isBookedAtShop),
  ...
  void _handleCancelClick({required BookingDetailResponse status, required bool isDurationMode}) {
    if (status.bookingDetail!.status == BookingStatusKeys.pending || status.bookingDetail!.status == BookingStatusKeys.accept || status.bookingDetail!.status == BookingStatusKeys.hold) {
      showInDialog(
        context,
        contentPadding: EdgeInsets.zero,
        insetPadding: isDurationMode && !status.service!.isFreeService && appConfigurationStore.cancellationCharge ? const EdgeInsets.symmetric(horizontal: 10) : null,
        builder: (context) {
          if (isDurationMode && !status.service!.isFreeService && appConfigurationStore.cancellationCharge && status.bookingDetail!.isAdvancePaymentDone) {
  ...
  Future<void> createCustomIcon() async {
    const ImageConfiguration imageConfiguration = ImageConfiguration(size: Size(24, 24));
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      stopLocationUpdates();
    } else if (state == AppLifecycleState.resumed && bookingStatus == BookingStatusKeys.onGoing) {
      refreshProviderLocation();
      startLocationUpdates();
    }
  }
